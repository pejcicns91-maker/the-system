...
# numeric pass
parts = [pd.read_parquet(f"ckpt_b0x/blk_{i:03d}.parquet", columns=['depth','kb','kt','n']) for i in range(NB)]
...
w = pq.ParquetWriter("bf_ladder_b0x.parquet", schema, compression='zstd')
digest = []
...
DG = pd.concat([DG[(DG.family==f)&(DG.depth==d)].nlargest(200,'lift')
                for f in ['bounce','through'] for d in [1,2,3,4]], ignore_index=True)
...
tot = len(n_)
print("=== B0x widened-ladder assembly complete ===")
...
for f in ["bf_ladder_b0x.parquet","bf_ladder_b0x_digest.csv","bf_extinction_d34.csv"]:

#!/usr/bin/env python3
"""e4e_commit.py — RECOVERY SHIP v4 (v3 + download retries with backoff; a CDN connection reset cost a tap). Root cause found and named: full-grid index files are ~212 MB, over GitHub's hard 100 MB per-file push limit — every in-tree ship since e4 died on the pre-receive hook, masked by truncated stderr. Amendment, per the c2 precedent (oversized record layers -> release, in-tree keeps decode/report/state): index_{COIN}.parquet ships as release 'c2x10' assets beside the cells; in-tree ships DECODE, REPORT, state (tiny, under the wall). Verification gates unchanged and re-run on this shipping run. for e4 (Svet's chat order, 2026-07-30). The e4 compute succeeded
...
def fetch(url,dst,tries=5):
    for i in range(tries):
        try: urllib.request.urlretrieve(url,dst); return True
            ...
            print(f'fetch retry {i+1}/{tries} {os.path.basename(dst)}: {type(e).__name__}',flush=True)
            try: os.remove(dst)
            except OSError: pass
            time.sleep(12*(i+1))
    return False
...
for coin in COINS:
    ...
    if not os.path.exists(zp) and not fetch(f'{R10}/cells10_{coin}.zip',zp):
        gate(f'{coin} B0 fetch',False,'cells10 download failed after 5 tries'); continue
    ...
    try:
        ...
        if not os.path.exists(zc) and not fetch(f'{RCB}/cells_{coin}.zip',zc): raise RuntimeError('c2 reference fetch failed')
    ...
    ixp=f'/tmp/index_{coin}.parquet'; IX.to_parquet(ixp,compression='zstd',index=False)
    if os.environ.get('GITHUB_ACTIONS'):
        ...
        r=subprocess.run(['gh','release','upload','c2x10',ixp,'--clobber'],capture_output=True,text=True)
        print(f'{coin} index -> release:','OK' if r.returncode==0 else 'FAILED\n'+r.stderr,flush=True)
...
open(f'{OUT}/DECODE.md','w').write("cells10_{COIN}_partNNN.parquet (release 'c2x10', concatenate parts in NNN order): cell = codeA*(nbB+1)+codeB; dial ids/bins in results/layerc2/dials_dict (identical grid, G2-gated on the compute run). Columns: n = anchors in cell; s_oV = win sum (NaN=0); n_oV for o3..o9 = finite count (o1/o2/o10 full universe: n_oV == n). rate = s/n_v. Floor n>=50 on total n (cell set identical to c2). index_{COIN}.parquet: release 'c2x10' asset (beside the cells; ~212 MB each, over the in-tree 100 MB wall). Per-pair cells, n_total (from pair_index; see REPORT provenance), sep_oV = n_v-weighted mean |rate - base_v|, bases sealed in results/state/e3.json.\n")
...
if os.environ.get('GITHUB_ACTIONS'):
    ...
    subprocess.run(['git','add','-A','results']); subprocess.run(['git','commit','-m','e4d: c2x10 docs+state [skip ci]'],capture_output=True)
    subprocess.run(['git','pull','--rebase'],capture_output=True); r=subprocess.run(['git','push'],capture_output=True,text=True)
    print('docs push:','OK' if r.returncode==0 else 'FAILED\n'+r.stderr,flush=True)

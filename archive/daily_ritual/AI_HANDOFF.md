...
## UPDATE 2026-07-15 — CB12 IS THE CHART LAYER
CB12_copilot.pine (plain-voice scenario copilot) replaces CB11 on the chart. Every morning run MUST execute CB12_PROTOCOL.md: the AI writes a 3-6 sentence MORNING READ (judgment, not template), composes per-wall zone plans (S-fields), a one-sentence NUANCE line (N:), and the day-type override (OVR:), then hands the finished CB12| payload lines to the user. Midday standing offer: "recheck {coin}" re-issues that zone plan from live data.
